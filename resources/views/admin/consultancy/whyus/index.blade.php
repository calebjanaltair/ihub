    ...
    <div class="col-lg-12">
        ...
        <div class="ibox">
            <div class="ibox-title d-flex justify-content-between align-items-center">
                <h5>Consultancy Why Us Points</h5>
                    ...
                    <table class="table table-striped table-bordered table-hover dataTables-example" id="tickersTable">
                        ...
                        <tbody id="ihub-news-records">
                            ...
                            <tr class="gradeX" style="cursor: pointer;">
                                ...
                                <td>{{\Illuminate\Support\Str::limit($whyus->title, 40, '...')}}</td>
                                <td>
                                    @if($whyus->status == 1)
                                    <span class="badge badge-primary">Active</span>
                                    @elseif($whyus->status == 0)
                                    <span class="badge badge-warning">Inactive</span>
                                    @endif
                                </td>

                                <td class="">
                                    <button type="button" class="btn btn-dark btn-sm btn-block" id="edit_button" data-bs-toggle="modal" data-bs-target="#editRecord" data-id="{{$whyus->id}}" data-title="{{$whyus->title}}" data-status="{{$whyus->status}}">
                                        <i class="fa fa-edit"> </i> Edit
                                    </button>
                                    @if($whyus->status == 0)
                                    <button class="btn btn-primary btn-sm btn-block activate-button" data-id="{{$whyus->id}}">
                                        <i class="fa fa-unlock"> </i> Activate
                                    </button>
                                    @elseif($whyus->status == 1)
                                    <button class="btn btn-warning btn-sm btn-block deactivate-button" data-id="{{$whyus->id}}">
                                        <i class="fa fa-lock"> </i> Deactivate
                                    </button>
                                    @endif
                                    <button class="btn btn-danger btn-sm btn-block delete-button" data-id="{{$whyus->id}}">
                                        <i class="fa fa-trash"> </i> Delete
                                    </button>
                                    <form action="{{ route('consultancy-whyus-delete') }}" id="delForm-{{$whyus->id}}" method="post">
                                        @csrf
                                        <input type="text" name="id" id="id" value="{{$whyus->id}}" hidden>
                                        <button type="submit" hidden>Submit</button>
                                    </form>
                                    <form action="{{ route('consultancy-whyus-activate') }}" id="activate-{{$whyus->id}}" method="post">
                                        @csrf
                                        <input type="text" name="id" id="id" value="{{$whyus->id}}" hidden>
                                        <button type="submit" hidden>Submit</button>
                                    </form>
                                    <form action="{{ route('consultancy-whyus-deactivate') }}" id="deactivate-{{$whyus->id}}" method="post">
                                        @csrf
                                        <input type="text" name="id" id="id" value="{{$whyus->id}}" hidden>
                                        <button type="submit" hidden>Submit</button>
                                    </form>
    ...
    <div class="modal-dialog modal-dialog-scrollable modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('consultancy-whyus-store') }}" method="post" enctype="multipart/form-data" id="addRecordForm">
                    @csrf
...
<div class="modal fade" id="editRecord" tabindex="-1" aria-labelledby="editRecord" aria-hidden="true">
    <div class="modal-dialog modal-dialog-scrollable modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('consultancy-whyus-update') }}" method="post" id="editRecordForm">
                    @csrf
