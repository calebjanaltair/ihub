    ...
    <div class="col-lg-12">
        ...
        <div class="ibox">
            <div class="ibox-title d-flex justify-content-between align-items-center">
                <h5>Board of Directors Cards Management</h5>
                    ...
                    <table class="table table-striped table-bordered table-hover dataTables-example" id="tickersTable">
                        ...
                        <tbody id="ihub-news-records">
                            ...
                            <tr class="gradeX" style="cursor: pointer;">
                                ...
                                <td class="">
                                    <button type="button" class="btn btn-dark btn-sm btn-block" id="edit_button" data-bs-toggle="modal" data-bs-target="#editRecord" data-id="{{$bod->id}}" data-name="{{$bod->name}}" data-title="{{$bod->title}}" data-image="{{$bod->image}}" data-link="{{$bod->link}}" data-status="{{$bod->status}}">
                                        <i class="fa fa-edit"> </i> Edit
                                    </button>
                                    @if($bod->status == 0)
                                    <button class="btn btn-primary btn-sm btn-block activate-button" data-id="{{$bod->id}}">
                                        <i class="fa fa-unlock"> </i> Activate
                                    </button>
                                    @elseif($bod->status == 1)
                                    <button class="btn btn-warning btn-sm btn-block deactivate-button" data-id="{{$bod->id}}">
                                        <i class="fa fa-lock"> </i> Deactivate
                                    </button>
                                    @endif
                                    <button class="btn btn-danger btn-sm btn-block delete-button" data-id="{{$bod->id}}">
                                        <i class="fa fa-trash"> </i> Delete
                                    </button>
                                    <form action="{{ route('bod-delete') }}" id="delForm-{{$bod->id}}" method="post">
                                        @csrf
                                        <input type="text" name="id" id="id" value="{{$bod->id}}" hidden>
                                        <button type="submit" hidden>Submit</button>
                                    </form>
                                    <form action="{{ route('bod-activate') }}" id="activate-{{$bod->id}}" method="post">
                                        @csrf
                                        <input type="text" name="id" id="id" value="{{$bod->id}}" hidden>
                                        <button type="submit" hidden>Submit</button>
                                    </form>
                                    <form action="{{ route('bod-deactivate') }}" id="deactivate-{{$bod->id}}" method="post">
                                        @csrf
                                        <input type="text" name="id" id="id" value="{{$bod->id}}" hidden>
                                        <button type="submit" hidden>Submit</button>
                                    </form>
    ...
    <div class="modal-dialog modal-dialog-scrollable modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('bod-store') }}" method="post" enctype="multipart/form-data" id="addRecordForm">
                    @csrf
...
<div class="modal fade" id="editRecord" tabindex="-1" aria-labelledby="editRecord" aria-hidden="true">
    <div class="modal-dialog modal-dialog-scrollable modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('bod-update') }}" method="post" enctype="multipart/form-data" id="editRecordForm">
                    @csrf
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#editRecord').on('show.bs.modal', function(event) {
            ...
            $('#edit_link').val(link);
            $('#status').val(status);
            $('#showEditImage').attr('src', './public/uploads/bod/' + image);
