    ...
    <div class="col-lg-12">
        ...
        <div class="ibox">
            <div class="ibox-title d-flex justify-content-between align-items-center">
                <h5>Collaboration Slides Management</h5>
                        ...
                        <tbody id="ihub-news-records">
                            ...
                            <tr class="gradeX" style="cursor: pointer;">
                                ...
                                <td>
                                    ...
                                    @elseif($colab->status == 0)
                                    <span class="badge badge-warning">Inactive</span>
                                    @endif
                                </td>

                                <td class="">
                                    <button type="button" class="btn btn-dark btn-sm btn-block" id="edit_button" data-bs-toggle="modal" data-bs-target="#editRecord" data-id="{{$colab->id}}" data-name="{{$colab->name}}" data-image="{{$colab->image}}" data-type="{{$colab->type}}" data-status="{{$colab->status}}">
                                        <i class="fa fa-edit"> </i> Edit
                                    </button>
                                    @if($colab->status == 0)
                                    <button class="btn btn-primary btn-sm btn-block activate-button" data-id="{{$colab->id}}">
                                        <i class="fa fa-unlock"> </i> Activate
                                    </button>
                                    @elseif($colab->status == 1)
                                    <button class="btn btn-warning btn-sm btn-block deactivate-button" data-id="{{$colab->id}}">
                                        <i class="fa fa-lock"> </i> Deactivate
                                    </button>
                                    @endif
                                    <button class="btn btn-danger btn-sm btn-block delete-button" data-id="{{$colab->id}}">
                                        <i class="fa fa-trash"> </i> Delete
                                    </button>
                                    <form action="{{ route('colab-delete') }}" id="delForm-{{$colab->id}}" method="post">
                                        @csrf
                                        <input type="text" name="id" id="id" value="{{$colab->id}}" hidden>
                                        <button type="submit" hidden>Submit</button>
                                    </form>
                                    <form action="{{ route('colab-activate') }}" id="activate-{{$colab->id}}" method="post">
                                        @csrf
                                        <input type="text" name="id" id="id" value="{{$colab->id}}" hidden>
                                        <button type="submit" hidden>Submit</button>
                                    </form>
                                    <form action="{{ route('colab-deactivate') }}" id="deactivate-{{$colab->id}}" method="post">
                                        @csrf
                                        <input type="text" name="id" id="id" value="{{$colab->id}}" hidden>
                                        <button type="submit" hidden>Submit</button>
                                    </form>
    ...
    <div class="modal-dialog modal-dialog-scrollable modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('colab-store') }}" method="post" enctype="multipart/form-data" id="addRecordForm">
                    @csrf
                    <div class="row mb-3 text-align-center align-items-center">
                        <img src="" class="d-none mx-auto my-auto" id="showImage" style="max-width: 500px; border-radius: 20px; border: 5px soolid gray;" alt="">
                    </div>
                    <div class="row mb-3">
                        <div class="col">
                            <div class="row mb-3">
                                ...
                                <div class="col">
                                    <h4><label for="exampleInputEmail1" class="form-label">Type</label></h4>
                                    <select class="form-select" name="type" id="type" aria-label="Default select example" required>
                                        <option disabled selected>Please Select an option</option>
                                        <option value="0">Collaboration Slide</option>
                                        <option value="1">Activities Glimpse</option>
                                    </select>
                            ...
                            <div class="row mb-3">
                                <div class="col">
                                    ...
                                    <div class="invalid-feedback">Image is required and must not exceed 2 MB.</div>
                                </div>
                            </div>
                        </div>
...
<div class="modal fade" id="editRecord" tabindex="-1" aria-labelledby="editRecord" aria-hidden="true">
    <div class="modal-dialog modal-dialog-scrollable modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('colab-update') }}" method="post" enctype="multipart/form-data" id="editRecordForm">
                    @csrf
                    <input type="text" name="id" id="edit_id" hidden>
                    <div class="row mb-3 text-align-center align-items-center">
                        <img src="" class="d-none mx-auto my-auto" id="showEditImage" style="max-width: 500px; border-radius: 20px;" alt="">
                    </div>
                    <div class="row mb-2">
                        <div class="col">
                            <div class="row mb-2">
                            ...
                            <div class="row mb-2">
                                <div class="col">
                                    <h4><label for="exampleInputEmail1" class="form-label">Type</label></h4>
                                    <select class="form-select" name="type" id="edit_type" aria-label="Default select example" required>
                                        <option disabled selected>Please Select an option</option>
                                        <option value="0">Collaboration Slide</option>
                                        <option value="1">Activities Glimpse</option>
                                    </select>
...
<script>
    ...
    function validateImage(input) {
        if (input.files[0].size > 50485760) {
