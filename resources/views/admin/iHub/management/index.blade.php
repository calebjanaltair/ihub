...
<div class="row">
    <div class="col-lg-12">
        ...
        <div class="alert alert-danger alert-dismissible fade show" role="alert">
            <strong>Updation Failed: </strong> {{session('error')}}.
            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
        </div>
        @endif
        <div class="ibox">
            <div class="ibox-title d-flex justify-content-between align-items-center">
                <h5>Managers Cards Management</h5>
                        ...
                        <tbody id="ihub-news-records">
                            ...
                            <tr class="gradeX" style="cursor: pointer;">
                                ...
                                <td>
                                    ...
                                    CEO
                                    @elseif ($manager->position == 1)
                                    SPM
                                    @elseif ($manager->position == 2)
                                    Manager
                                    @elseif ($manager->position == 3)
                                    Deputy Manager
                                    @elseif ($manager->position == 4)
                                    Associate Manager
                                    @elseif ($manager->position == 5)
                                    Associate Admin
                                    @elseif ($manager->position == 6)
                                    Associate
                                    @else
                                    Unknown Position
                                    @endif
                                ...
                                <td class="">
                                    <button type="button" class="btn btn-dark btn-sm btn-block" id="edit_button" data-bs-toggle="modal" data-bs-target="#editRecord" data-id="{{$manager->id}}" data-name="{{$manager->name}}" data-title="{{$manager->title}}" data-image="{{$manager->image}}" data-link="{{$manager->link}}" data-position="{{$manager->position}}" data-status="{{$manager->status}}">
                                        <i class="fa fa-edit"> </i> Edit
                                    </button>
                                    @if($manager->status == 0)
                                    <button class="btn btn-primary btn-sm btn-block activate-button" data-id="{{$manager->id}}">
                                        <i class="fa fa-unlock"> </i> Activate
                                    </button>
                                    @elseif($manager->status == 1)
                                    <button class="btn btn-warning btn-sm btn-block deactivate-button" data-id="{{$manager->id}}">
                                        <i class="fa fa-lock"> </i> Deactivate
                                    </button>
                                    @endif
                                    <button class="btn btn-danger btn-sm btn-block delete-button" data-id="{{$manager->id}}">
                                        <i class="fa fa-trash"> </i> Delete
                                    </button>
                                    <form action="{{ route('management-delete') }}" id="delForm-{{$manager->id}}" method="post">
                                        @csrf
                                        <input type="text" name="id" id="id" value="{{$manager->id}}" hidden>
                                        <button type="submit" hidden>Submit</button>
                                    </form>
                                    <form action="{{ route('management-activate') }}" id="activate-{{$manager->id}}" method="post">
                                        @csrf
                                        <input type="text" name="id" id="id" value="{{$manager->id}}" hidden>
                                        <button type="submit" hidden>Submit</button>
                                    </form>
                                    <form action="{{ route('management-deactivate') }}" id="deactivate-{{$manager->id}}" method="post">
                                        @csrf
                                        <input type="text" name="id" id="id" value="{{$manager->id}}" hidden>
                                        <button type="submit" hidden>Submit</button>
                                    </form>
    ...
    <div class="modal-dialog modal-dialog-scrollable modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('management-store') }}" method="post" enctype="multipart/form-data" id="addRecordForm">
                    @csrf
                    ...
                    <div class="row mb-3">
                        <div class="col">
                            ...
                            <div class="row mb-3">
                                <div class="col">
                                    <h4><label for="exampleInputEmail1" class="form-label">Positon</label></h4>
                                    <select class="form-select" required name="position" id="position" aria-label="Default select example">
                                        {!! postion_options() !!}
                                    </select>
                                </div>
                                <div class="col">
...
<div class="modal fade" id="editRecord" tabindex="-1" aria-labelledby="editRecord" aria-hidden="true">
    <div class="modal-dialog modal-dialog-scrollable modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('management-update') }}" method="post" enctype="multipart/form-data" id="editRecordForm">
                    @csrf
                    ...
                    <div class="row mb-2">
                        <div class="col">
                            <div class="row mb-2">
                                ...
                                <!-- <div class="col">
                                    <h4><label for="exampleInputEmail1" class="form-label">Positon</label></h4>
                                    <select class="form-select" required name="position" id="edit_position" aria-label="Default select example">
                                        {!! edit_postion_options() !!}
                                    </select>
                                </div> -->
                                <div class="col">
