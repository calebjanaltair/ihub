    ...
    <div class="col-lg-12">
        ...
        <div class="ibox">
            <div class="ibox-title d-flex justify-content-between align-items-center">
                <h5>Hub Governance Board Cards Management</h5>
                    ...
                    <table class="table table-striped table-bordered table-hover dataTables-example" id="tickersTable">
                        ...
                        <tbody id="ihub-news-records">
                            ...
                            <tr class="gradeX" style="cursor: pointer;">
                                ...
                                <td class="">
                                    <button type="button" class="btn btn-dark btn-sm btn-block" id="edit_button" data-bs-toggle="modal" data-bs-target="#editRecord" data-id="{{$hgb->id}}" data-name="{{$hgb->name}}" data-title="{{$hgb->title}}" data-image="{{$hgb->image}}" data-link="{{$hgb->link}}" data-status="{{$hgb->status}}">
                                        <i class="fa fa-edit"> </i> Edit
                                    </button>
                                    @if($hgb->status == 0)
                                    <button class="btn btn-primary btn-sm btn-block activate-button" data-id="{{$hgb->id}}">
                                        <i class="fa fa-unlock"> </i> Activate
                                    </button>
                                    @elseif($hgb->status == 1)
                                    <button class="btn btn-warning btn-sm btn-block deactivate-button" data-id="{{$hgb->id}}">
                                        <i class="fa fa-lock"> </i> Deactivate
                                    </button>
                                    @endif
                                    <button class="btn btn-danger btn-sm btn-block delete-button" data-id="{{$hgb->id}}">
                                        <i class="fa fa-trash"> </i> Delete
                                    </button>
                                    <form action="{{ route('hgb-delete') }}" id="delForm-{{$hgb->id}}" method="post">
                                        @csrf
                                        <input type="text" name="id" id="id" value="{{$hgb->id}}" hidden>
                                        <button type="submit" hidden>Submit</button>
                                    </form>
                                    <form action="{{ route('hgb-activate') }}" id="activate-{{$hgb->id}}" method="post">
                                        @csrf
                                        <input type="text" name="id" id="id" value="{{$hgb->id}}" hidden>
                                        <button type="submit" hidden>Submit</button>
                                    </form>
                                    <form action="{{ route('hgb-deactivate') }}" id="deactivate-{{$hgb->id}}" method="post">
                                        @csrf
                                        <input type="text" name="id" id="id" value="{{$hgb->id}}" hidden>
                                        <button type="submit" hidden>Submit</button>
                                    </form>
    ...
    <div class="modal-dialog modal-dialog-scrollable modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('hgb-store') }}" method="post" enctype="multipart/form-data" id="addRecordForm">
                    @csrf
...
<div class="modal fade" id="editRecord" tabindex="-1" aria-labelledby="editRecord" aria-hidden="true">
    <div class="modal-dialog modal-dialog-scrollable modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('hgb-update') }}" method="post" enctype="multipart/form-data" id="editRecordForm">
                    @csrf
