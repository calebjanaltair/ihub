    ...
    <div class="col-lg-12">
        ...
        <div class="ibox">
            <div class="ibox-title d-flex justify-content-between align-items-center">
                <h5>What's New Cards Management</h5>
                        ...
                        <tbody id="ihub-news-records">
                            ...
                            <tr class="gradeX" style="cursor: pointer;">
                                ...
                                <td>
                                    ...
                                    @elseif($whatnew->status == 0)
                                    <span class="badge badge-warning">Inactive</span>
                                    @endif
                                </td>

                                <td class="">
                                    <button type="button" class="btn btn-dark btn-sm btn-block" id="edit_button" data-bs-toggle="modal" data-bs-target="#editRecord" data-id="{{$whatnew->id}}" data-name="{{$whatnew->name}}" data-link="{{$whatnew->link}}" data-image="{{$whatnew->image}}" data-status="{{$whatnew->status}}">
                                        <i class="fa fa-edit"> </i> Edit
                                    </button>
                                    @if($whatnew->status == 0)
                                    <button class="btn btn-primary btn-sm btn-block activate-button" data-id="{{$whatnew->id}}">
                                        <i class="fa fa-unlock"> </i> Activate
                                    </button>
                                    @elseif($whatnew->status == 1)
                                    <button class="btn btn-warning btn-sm btn-block deactivate-button" data-id="{{$whatnew->id}}">
                                        <i class="fa fa-lock"> </i> Deactivate
                                    </button>
                                    @endif
                                    <button class="btn btn-danger btn-sm btn-block delete-button" data-id="{{$whatnew->id}}">
                                        <i class="fa fa-trash"> </i> Delete
                                    </button>
                                    <form action="{{ route('what_new-delete') }}" id="delForm-{{$whatnew->id}}" method="post">
                                        @csrf
                                        <input type="text" name="id" id="id" value="{{$whatnew->id}}" hidden>
                                        <button type="submit" hidden>Submit</button>
                                    </form>
                                    <form action="{{ route('what_new-activate') }}" id="activate-{{$whatnew->id}}" method="post">
                                        @csrf
                                        <input type="text" name="id" id="id" value="{{$whatnew->id}}" hidden>
                                        <button type="submit" hidden>Submit</button>
                                    </form>
                                    <form action="{{ route('what_new-deactivate') }}" id="deactivate-{{$whatnew->id}}" method="post">
                                        @csrf
                                        <input type="text" name="id" id="id" value="{{$whatnew->id}}" hidden>
                                        <button type="submit" hidden>Submit</button>
                                    </form>
    ...
    <div class="modal-dialog modal-dialog-scrollable modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('what_new-store') }}" method="post" enctype="multipart/form-data" id="addRecordForm">
                    @csrf
...
<div class="modal fade" id="editRecord" tabindex="-1" aria-labelledby="editRecord" aria-hidden="true">
    <div class="modal-dialog modal-dialog-scrollable modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('what_new-update') }}" method="post" enctype="multipart/form-data" id="editRecordForm">
                    @csrf
