    ...
    <div class="col-lg-12">
        ...
        <div class="ibox">
            <div class="ibox-title d-flex justify-content-between align-items-center">
                <h5>Gallery Slides Management</h5>
                    ...
                    <table class="table table-striped table-bordered table-hover dataTables-example" id="tickersTable">
                        ...
                        <tbody id="ihub-news-records">
                            ...
                            <tr class="gradeX" style="cursor: pointer;">
                                ...
                                <td class="">
                                    <button type="button" class="btn btn-dark btn-sm btn-block" id="edit_button" data-bs-toggle="modal" data-bs-target="#editRecord" data-id="{{$gallery->id}}" data-name="{{$gallery->name}}" data-image="{{$gallery->image}}" data-status="{{$gallery->status}}">
                                        <i class="fa fa-edit"> </i> Edit
                                    </button>
                                    @if($gallery->status == 0)
                                    <button class="btn btn-primary btn-sm btn-block activate-button" data-id="{{$gallery->id}}">
                                        <i class="fa fa-unlock"> </i> Activate
                                    </button>
                                    @elseif($gallery->status == 1)
                                    <button class="btn btn-warning btn-sm btn-block deactivate-button" data-id="{{$gallery->id}}">
                                        <i class="fa fa-lock"> </i> Deactivate
                                    </button>
                                    @endif
                                    <button class="btn btn-danger btn-sm btn-block delete-button" data-id="{{$gallery->id}}">
                                        <i class="fa fa-trash"> </i> Delete
                                    </button>
                                    <form action="{{ route('galleria-delete') }}" id="delForm-{{$gallery->id}}" method="post">
                                        @csrf
                                        <input type="text" name="id" id="id" value="{{$gallery->id}}" hidden>
                                        <button type="submit" hidden>Submit</button>
                                    </form>
                                    <form action="{{ route('galleria-activate') }}" id="activate-{{$gallery->id}}" method="post">
                                        @csrf
                                        <input type="text" name="id" id="id" value="{{$gallery->id}}" hidden>
                                        <button type="submit" hidden>Submit</button>
                                    </form>
                                    <form action="{{ route('galleria-deactivate') }}" id="deactivate-{{$gallery->id}}" method="post">
                                        @csrf
                                        <input type="text" name="id" id="id" value="{{$gallery->id}}" hidden>
                                        <button type="submit" hidden>Submit</button>
                                    </form>
    ...
    <div class="modal-dialog modal-dialog-scrollable modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('galleria-store') }}" method="post" enctype="multipart/form-data" id="addRecordForm">
                    @csrf
...
<div class="modal fade" id="editRecord" tabindex="-1" aria-labelledby="editRecord" aria-hidden="true">
    <div class="modal-dialog modal-dialog-scrollable modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('galleria-update') }}" method="post" enctype="multipart/form-data" id="editRecordForm">
                    @csrf
