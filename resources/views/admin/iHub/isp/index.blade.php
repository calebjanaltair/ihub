    ...
    <div class="col-lg-12">
        ...
        <div class="ibox">
            <div class="ibox-title d-flex justify-content-between align-items-center">
                <h5>Innovation Support Programs Management</h5>
                    ...
                    <table class="table table-striped table-bordered table-hover dataTables-example" id="tickersTable">
                        ...
                        <tbody id="ihub-news-records">
                            ...
                            <tr class="gradeX" style="cursor: pointer;">
                                ...
                                <td class="">
                                    <button type="button" class="btn btn-dark btn-sm btn-block" id="edit_button" data-bs-toggle="modal" data-bs-target="#editRecord" data-id="{{$isp->id}}" data-name="{{$isp->name}}" data-image="{{$isp->image}}" data-description="{{$isp->description}}" data-status="{{$isp->status}}">
                                        <i class="fa fa-edit"> </i> Edit
                                    </button>
                                    @if($isp->status == 0)
                                    <button class="btn btn-primary btn-sm btn-block activate-button" data-id="{{$isp->id}}">
                                        <i class="fa fa-unlock"> </i> Activate
                                    </button>
                                    @elseif($isp->status == 1)
                                    <button class="btn btn-warning btn-sm btn-block deactivate-button" data-id="{{$isp->id}}">
                                        <i class="fa fa-lock"> </i> Deactivate
                                    </button>
                                    @endif
                                    <button class="btn btn-danger btn-sm btn-block delete-button" data-id="{{$isp->id}}">
                                        <i class="fa fa-trash"> </i> Delete
                                    </button>
                                    <form action="{{ route('isprogram-delete') }}" id="delForm-{{$isp->id}}" method="post">
                                        @csrf
                                        <input type="text" name="id" id="id" value="{{$isp->id}}" hidden>
                                        <button type="submit" hidden>Submit</button>
                                    </form>
                                    <form action="{{ route('isprogram-activate') }}" id="activate-{{$isp->id}}" method="post">
                                        @csrf
                                        <input type="text" name="id" id="id" value="{{$isp->id}}" hidden>
                                        <button type="submit" hidden>Submit</button>
                                    </form>
                                    <form action="{{ route('isprogram-deactivate') }}" id="deactivate-{{$isp->id}}" method="post">
                                        @csrf
                                        <input type="text" name="id" id="id" value="{{$isp->id}}" hidden>
                                        <button type="submit" hidden>Submit</button>
                                    </form>
    ...
    <div class="modal-dialog modal-dialog-scrollable modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('isprogram-store') }}" method="post" enctype="multipart/form-data" id="addRecordForm">
                    @csrf
...
<div class="modal fade" id="editRecord" tabindex="-1" aria-labelledby="editRecord" aria-hidden="true">
    <div class="modal-dialog modal-dialog-scrollable modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('isprogram-update') }}" method="post" enctype="multipart/form-data" id="editRecordForm">
                    @csrf
                    ...
                    <div class="row mb-2">
                        <div class="col">
                            ...
                            <div class="row mb-2">
                                <div class="col">
                                    <h4><label for="exampleInputEmail1" class="form-label">Description</label></h4>
                                    <textarea class="form-control summernote" name="description" id="edit_description" rows="3" required></textarea>
                                    <div class="invalid-feedback">Description is required.</div>
                                </div>
                            </div>
                        </div>
