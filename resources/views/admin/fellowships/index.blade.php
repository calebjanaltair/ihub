    ...
    <div class="col-lg-12">
        ...
        <div class="ibox">
            <div class="ibox-title d-flex justify-content-between align-items-center">
                <h5>Fellowship Programs Management</h5>
                    ...
                    <table class="table table-striped table-bordered table-hover dataTables-example" id="tickersTable">
                        ...
                        <tbody id="ihub-news-records">
                            ...
                            <tr class="gradeX" style="cursor: pointer;">
                                ...
                                <td>
                                    @if($fellowship->status == 1)
                                    <span class="badge badge-primary">Active</span>
                                    @elseif($fellowship->status == 0)
                                    <span class="badge badge-warning">Inactive</span>
                                    @endif
                                </td>

                                <td class="">
                                    <button type="button" class="btn btn-dark btn-sm btn-block" id="edit_button" data-bs-toggle="modal" data-bs-target="#editRecord" data-id="{{$fellowship->id}}" data-name="{{$fellowship->name}}" data-link="{{$fellowship->link}}" data-image="{{$fellowship->image}}" data-brochure="{{$fellowship->brochure}}" data-description="{{$fellowship->description}}" data-status="{{$fellowship->status}}">
                                        <i class="fa fa-edit"> </i> Edit
                                    </button>
                                    @if($fellowship->status == 0)
                                    <button class="btn btn-primary btn-sm btn-block activate-button" data-id="{{$fellowship->id}}">
                                        <i class="fa fa-unlock"> </i> Activate
                                    </button>
                                    @elseif($fellowship->status == 1)
                                    <button class="btn btn-warning btn-sm btn-block deactivate-button" data-id="{{$fellowship->id}}">
                                        <i class="fa fa-lock"> </i> Deactivate
                                    </button>
                                    @endif
                                    <button class="btn btn-danger btn-sm btn-block delete-button" data-id="{{$fellowship->id}}">
                                        <i class="fa fa-trash"> </i> Delete
                                    </button>
                                    <form action="{{ route('fellowship-programs-delete') }}" id="delForm-{{$fellowship->id}}" method="post">
                                        @csrf
                                        <input type="text" name="id" id="id" value="{{$fellowship->id}}" hidden>
                                        <button type="submit" hidden>Submit</button>
                                    </form>
                                    <form action="{{ route('fellowship-programs-activate') }}" id="activate-{{$fellowship->id}}" method="post">
                                        @csrf
                                        <input type="text" name="id" id="id" value="{{$fellowship->id}}" hidden>
                                        <button type="submit" hidden>Submit</button>
                                    </form>
                                    <form action="{{ route('fellowship-programs-deactivate') }}" id="deactivate-{{$fellowship->id}}" method="post">
                                        @csrf
                                        <input type="text" name="id" id="id" value="{{$fellowship->id}}" hidden>
                                        <button type="submit" hidden>Submit</button>
                                    </form>
    ...
    <div class="modal-dialog modal-dialog-scrollable modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('fellowship-programs-store') }}" method="post" enctype="multipart/form-data" id="addRecordForm">
                    @csrf
                    ...
                    <div class="row mb-3">
                        <div class="col">
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col">
                                    <h4><label for="exampleInputEmail1" class="form-label">Brochure</label></h4>
                                    <input type="file" class="form-control" name="brochure" id="brochure" required>
                                    <div class="invalid-feedback">Brochure is required and must not exceed 1 MB.</div>
                                </div>
                            </div>
                            <div class="row mb-3">
                                <div class="col">
...
<div class="modal fade" id="editRecord" tabindex="-1" aria-labelledby="editRecord" aria-hidden="true">
    <div class="modal-dialog modal-dialog-scrollable modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('fellowship-programs-update') }}" method="post" enctype="multipart/form-data" id="editRecordForm">
                    @csrf
                    ...
                    <div class="row mb-3">
                        <div class="col">
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col">
                                    <h4><label for="exampleInputEmail1" class="form-label">Brochure</label></h4>
                                    <input type="file" class="form-control" name="brochure" id="edit_brochure" required>
                                    <div class="invalid-feedback">Brochure is required and must not exceed 1 MB.</div>
                                </div>
                            </div>
                            <div class="row mb-3">
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#editRecord').on('show.bs.modal', function(event) {
            ...
            $('#edit_link').val(link);
            $('#showEditImage').attr('src', '{{asset("/public/uploads/fellowships/poster/")}}' + '/' + image);
