    ...
    <div class="col-lg-12">
        ...
        <div class="ibox">
            <div class="ibox-title d-flex justify-content-between align-items-center">
                <h5>Training News & Updates Cards Management</h5>
                    ...
                    <table class="table table-striped table-bordered table-hover dataTables-example" id="tickersTable">
                        ...
                        <tbody id="ihub-news-records">
                            ...
                            <tr class="gradeX" style="cursor: pointer;">
                                ...
                                <td class="">
                                    <button type="button" class="btn btn-dark btn-sm btn-block" id="edit_button" data-bs-toggle="modal" data-bs-target="#editRecord" data-id="{{$news->id}}" data-name="{{$news->name}}" data-title="{{$news->title}}" data-image="{{$news->image}}" data-description="{{$news->description}}" data-status="{{$news->status}}">
                                        <i class="fa fa-edit"> </i> Edit
                                    </button>
                                    @if($news->status == 0)
                                    <button class="btn btn-primary btn-sm btn-block activate-button" data-id="{{$news->id}}">
                                        <i class="fa fa-unlock"> </i> Activate
                                    </button>
                                    @elseif($news->status == 1)
                                    <button class="btn btn-warning btn-sm btn-block deactivate-button" data-id="{{$news->id}}">
                                        <i class="fa fa-lock"> </i> Deactivate
                                    </button>
                                    @endif
                                    <button class="btn btn-danger btn-sm btn-block delete-button" data-id="{{$news->id}}">
                                        <i class="fa fa-trash"> </i> Delete
                                    </button>
                                    <form action="{{ route('AN-delete') }}" id="delForm-{{$news->id}}" method="post">
                                        @csrf
                                        <input type="text" name="id" id="id" value="{{$news->id}}" hidden>
                                        <button type="submit" hidden>Submit</button>
                                    </form>
                                    <form action="{{ route('AN-activate') }}" id="activate-{{$news->id}}" method="post">
                                        @csrf
                                        <input type="text" name="id" id="id" value="{{$news->id}}" hidden>
                                        <button type="submit" hidden>Submit</button>
                                    </form>
                                    <form action="{{ route('AN-deactivate') }}" id="deactivate-{{$news->id}}" method="post">
                                        @csrf
                                        <input type="text" name="id" id="id" value="{{$news->id}}" hidden>
                                        <button type="submit" hidden>Submit</button>
                                    </form>
    ...
    <div class="modal-dialog modal-dialog-scrollable modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('AN-store') }}" method="post" enctype="multipart/form-data" id="addRecordForm">
                    @csrf
...
<div class="modal fade" id="editRecord" tabindex="-1" aria-labelledby="editRecord" aria-hidden="true">
    <div class="modal-dialog modal-dialog-scrollable modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('AN-update') }}" method="post" enctype="multipart/form-data" id="editRecordForm">
                    @csrf
