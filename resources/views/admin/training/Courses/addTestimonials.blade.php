    ...
    <div class="col-lg-12">
        ...
        <div class="ibox">
            <div class="ibox-title d-flex justify-content-between align-items-center">
                ...
                <div class="col-sm-8">
                    <a class="btn btn-success float-end" href="{{route('course-transitions', ['id' => $courseID])}}">
                        Save & Next
                    </a>
                </div>
            </div>
            <div class="ibox-title d-flex justify-content-between align-items-center">
                <h5>Course Testimonials Management</h5>
                ...
                <div class="table-responsive">
                    <table class="table table-striped table-bordered table-hover dataTables-example" id="tickersTable">
                        ...
                        <td class="">
                            ...
                            <form action="{{ route('course-testimonials-delete') }}" id="delForm-{{$testimonial->id}}" method="post">
                                @csrf
                                <input type="text" name="id" id="id" value="{{$testimonial->id}}" hidden>
                                <button type="submit" hidden>Submit</button>
                            </form>
                            <form action="{{ route('course-testimonials-activate') }}" id="activate-{{$testimonial->id}}" method="post">
                                @csrf
                                <input type="text" name="id" id="id" value="{{$testimonial->id}}" hidden>
                                <button type="submit" hidden>Submit</button>
                            </form>
                            <form action="{{ route('course-testimonials-deactivate') }}" id="deactivate-{{$testimonial->id}}" method="post">
                                @csrf
                                <input type="text" name="id" id="id" value="{{$testimonial->id}}" hidden>
                                <button type="submit" hidden>Submit</button>
                            </form>
    ...
    <div class="modal-dialog modal-dialog-scrollable modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('course-testimonials-store') }}" method="post" enctype="multipart/form-data" id="addRecordForm">
                    @csrf
...
<div class="modal fade" id="editRecord" tabindex="-1" aria-labelledby="editRecord" aria-hidden="true">
    <div class="modal-dialog modal-dialog-scrollable modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('course-testimonials-update') }}" method="post" enctype="multipart/form-data" id="editRecordForm">
                    @csrf
                    <input type="text" name="course_id" value="{{$courseID}}" hidden>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#editRecord').on('show.bs.modal', function(event) {
            ...
            $('#showEditImage').attr('src', '{{asset("/public/uploads/CourseTestimonials/")}}' + '/' + image);
