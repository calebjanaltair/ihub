    ...
    <div class="col-lg-12">
        ...
        <div class="ibox">
            <div class="ibox-title d-flex justify-content-between align-items-center">
                <div class="col-sm-4">
                    <h2>Add Course Transitions</h2>
                </div>
                <div class="col-sm-8">
                    <a class="btn btn-success float-end" href="{{route('cwca', ['id' => $courseID])}}">
                        Save & Next
                    </a>
                </div>
            </div>
            <div class="ibox-title d-flex justify-content-between align-items-center">
                <h5>Course Career Transition Management</h5>
                        ...
                        <tbody id="ihub-news-records">
                            ...
                            <tr class="gradeX" style="cursor: pointer;">
                                ...
                                <td>
                                    ...
                                    @elseif($transition->status == 0)
                                    <span class="badge badge-warning">Inactive</span>
                                    @endif
                                </td>

                                <td class="">
                                    <button type="button" class="btn btn-dark btn-sm btn-block" id="edit_button" data-bs-toggle="modal" data-bs-target="#editRecord" data-id="{{$transition->id}}" data-name="{{$transition->name}}" data-title="{{$transition->title}}" data-image="{{$transition->image}}" data-description="{{$transition->description}}" data-status="{{$transition->status}}">
                                        <i class="fa fa-edit"> </i> Edit
                                    </button>
                                    @if($transition->status == 0)
                                    <button class="btn btn-primary btn-sm btn-block activate-button" data-id="{{$transition->id}}">
                                        <i class="fa fa-unlock"> </i> Activate
                                    </button>
                                    @elseif($transition->status == 1)
                                    <button class="btn btn-warning btn-sm btn-block deactivate-button" data-id="{{$transition->id}}">
                                        <i class="fa fa-lock"> </i> Deactivate
                                    </button>
                                    @endif
                                    <button class="btn btn-danger btn-sm btn-block delete-button" data-id="{{$transition->id}}">
                                        <i class="fa fa-trash"> </i> Delete
                                    </button>
                                    <form action="{{ route('course-transitions-delete') }}" id="delForm-{{$transition->id}}" method="post">
                                        @csrf
                                        <input type="text" name="id" id="id" value="{{$transition->id}}" hidden>
                                        <button type="submit" hidden>Submit</button>
                                    </form>
                                    <form action="{{ route('course-transitions-activate') }}" id="activate-{{$transition->id}}" method="post">
                                        @csrf
                                        <input type="text" name="id" id="id" value="{{$transition->id}}" hidden>
                                        <button type="submit" hidden>Submit</button>
                                    </form>
                                    <form action="{{ route('course-transitions-deactivate') }}" id="deactivate-{{$transition->id}}" method="post">
                                        @csrf
                                        <input type="text" name="id" id="id" value="{{$transition->id}}" hidden>
                                        <button type="submit" hidden>Submit</button>
                                    </form>
    ...
    <div class="modal-dialog modal-dialog-scrollable modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('course-transitions-store') }}" method="post" enctype="multipart/form-data" id="addRecordForm">
                    @csrf
...
<div class="modal fade" id="editRecord" tabindex="-1" aria-labelledby="editRecord" aria-hidden="true">
    <div class="modal-dialog modal-dialog-scrollable modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('course-transitions-update') }}" method="post" enctype="multipart/form-data" id="editRecordForm">
                    @csrf
                    <input type="text" name="course_id" value="{{$courseID}}" hidden>
