    ...
    <div class="col-lg-12">
        ...
        <div class="ibox">
            <div class="ibox-title d-flex justify-content-between align-items-center">
                <div class="col-sm-4">
                    <h2>Add Key Highlights of Course</h2>
                </div>
                <div class="col-sm-8">
                    <a class="btn btn-success float-end" href="{{route('save&publish', ['id' => $courseID])}}">
                        Save & Publish
                    </a>
                </div>
            </div>
            <div class="ibox-title d-flex justify-content-between align-items-center">
                <h5>Key Highlights Management</h5>
                    ...
                    <table class="table table-striped table-bordered table-hover dataTables-example" id="tickersTable">
                        ...
                        <tbody id="ihub-news-records">
                            ...
                            <tr class="gradeX" style="cursor: pointer;">
                                ...
                                <td class="">
                                    <button type="button" class="btn btn-dark btn-sm btn-block" id="edit_button" data-bs-toggle="modal" data-bs-target="#editRecord" data-id="{{$highlight->id}}" data-name="{{$highlight->name}}" data-status="{{$highlight->status}}">
                                        <i class="fa fa-edit"> </i> Edit
                                    </button>
                                    @if($highlight->status == 0)
                                    <button class="btn btn-primary btn-sm btn-block activate-button" data-id="{{$highlight->id}}">
                                        <i class="fa fa-unlock"> </i> Activate
                                    </button>
                                    @elseif($highlight->status == 1)
                                    <button class="btn btn-warning btn-sm btn-block deactivate-button" data-id="{{$highlight->id}}">
                                        <i class="fa fa-lock"> </i> Deactivate
                                    </button>
                                    @endif
                                    <button class="btn btn-danger btn-sm btn-block delete-button" data-id="{{$highlight->id}}">
                                        <i class="fa fa-trash"> </i> Delete
                                    </button>
                                    <form action="{{ route('highlights-delete') }}" id="delForm-{{$highlight->id}}" method="post">
                                        @csrf
                                        <input type="text" name="id" id="id" value="{{$highlight->id}}" hidden>
                                        <button type="submit" hidden>Submit</button>
                                    </form>
                                    <form action="{{ route('highlights-activate') }}" id="activate-{{$highlight->id}}" method="post">
                                        @csrf
                                        <input type="text" name="id" id="id" value="{{$highlight->id}}" hidden>
                                        <button type="submit" hidden>Submit</button>
                                    </form>
                                    <form action="{{ route('highlights-deactivate') }}" id="deactivate-{{$highlight->id}}" method="post">
                                        @csrf
                                        <input type="text" name="id" id="id" value="{{$highlight->id}}" hidden>
                                        <button type="submit" hidden>Submit</button>
                                    </form>
    ...
    <div class="modal-dialog modal-dialog-scrollable modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('highlights-store') }}" method="post" enctype="multipart/form-data" id="addRecordForm">
                    @csrf
...
<div class="modal fade" id="editRecord" tabindex="-1" aria-labelledby="editRecord" aria-hidden="true">
    <div class="modal-dialog modal-dialog-scrollable modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('highlights-update') }}" method="post" enctype="multipart/form-data" id="editRecordForm">
                    @csrf
                    <input type="text" name="course_id" value="{{$courseID}}" hidden>
