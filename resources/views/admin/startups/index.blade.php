    ...
    <div class="col-lg-12">
        ...
        <div class="ibox">
            <div class="ibox-title d-flex justify-content-between align-items-center">
                <h5>Startup Programs Management</h5>
                        ...
                        <tbody id="ihub-news-records">
                            ...
                            <tr class="gradeX" style="cursor: pointer;">
                                ...
                                <td>
                                    ...
                                    @elseif($startup->status == 0)
                                    <span class="badge badge-warning">Inactive</span>
                                    @endif
                                </td>

                                <td class="">
                                    <button type="button" class="btn btn-dark btn-sm btn-block" id="edit_button" data-bs-toggle="modal" data-bs-target="#editRecord" data-id="{{$startup->id}}" data-name="{{$startup->name}}" data-link="{{$startup->link}}" data-image="{{$startup->image}}" data-brochure="{{$startup->brochure}}" data-description="{{$startup->description}}" data-status="{{$startup->status}}">
                                        <i class="fa fa-edit"> </i> Edit
                                    </button>
                                    @if($startup->status == 0)
                                    <button class="btn btn-primary btn-sm btn-block activate-button" data-id="{{$startup->id}}">
                                        <i class="fa fa-unlock"> </i> Activate
                                    </button>
                                    @elseif($startup->status == 1)
                                    <button class="btn btn-warning btn-sm btn-block deactivate-button" data-id="{{$startup->id}}">
                                        <i class="fa fa-lock"> </i> Deactivate
                                    </button>
                                    @endif
                                    <button class="btn btn-danger btn-sm btn-block delete-button" data-id="{{$startup->id}}">
                                        <i class="fa fa-trash"> </i> Delete
                                    </button>
                                    <form action="{{ route('startup-programs-delete') }}" id="delForm-{{$startup->id}}" method="post">
                                        @csrf
                                        <input type="text" name="id" id="id" value="{{$startup->id}}" hidden>
                                        <button type="submit" hidden>Submit</button>
                                    </form>
                                    <form action="{{ route('startup-programs-activate') }}" id="activate-{{$startup->id}}" method="post">
                                        @csrf
                                        <input type="text" name="id" id="id" value="{{$startup->id}}" hidden>
                                        <button type="submit" hidden>Submit</button>
                                    </form>
                                    <form action="{{ route('startup-programs-deactivate') }}" id="deactivate-{{$startup->id}}" method="post">
                                        @csrf
                                        <input type="text" name="id" id="id" value="{{$startup->id}}" hidden>
                                        <button type="submit" hidden>Submit</button>
                                    </form>
    ...
    <div class="modal-dialog modal-dialog-scrollable modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('startup-programs-store') }}" method="post" enctype="multipart/form-data" id="addRecordForm">
                    @csrf
...
<div class="modal fade" id="editRecord" tabindex="-1" aria-labelledby="editRecord" aria-hidden="true">
    <div class="modal-dialog modal-dialog-scrollable modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('startup-programs-update') }}" method="post" enctype="multipart/form-data" id="editRecordForm">
                    @csrf
                    ...
                    <div class="row mb-3">
                        <div class="col">
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col">
                                    <h4><label for="exampleInputEmail1" class="form-label">Brochure</label></h4>
                                    <input type="file" class="form-control" name="brochure" id="edit_brochure">
                                    <div class="invalid-feedback">Brochure is required and must not exceed 1 MB.</div>
                                </div>
                            </div>
                            <div class="row mb-3">
